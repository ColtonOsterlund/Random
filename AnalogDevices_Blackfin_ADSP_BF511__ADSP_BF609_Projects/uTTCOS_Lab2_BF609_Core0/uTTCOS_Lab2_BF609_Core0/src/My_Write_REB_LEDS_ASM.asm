...
.global _My_Write_REB_LEDS_ASM; //extern "C" unsigned short int My_Write_REB_LEDS_CPP(unsigned short int valueToWrite){
...
//portPointer_P0.H = hi(REG_PORTF_DATA); portPointer_P0.L = lo(REG_PORTF_DATA); //portPattern = *pREG_PORTF_DATA; - port address is 32 bits, port itself is 16 btis
	...
	//portMask_R1.L = lo(0xf000); portMask_R1.H = hi(0xf000);
	//portMask_R2.L = lo(0x0fff); portMask_R2.H = hi(0xf000);
	R1.L = lo(0xf000); R1.H = hi(0x0000);
	R2.L = lo(0x0fff); R2.H = hi(0x0000);
	
	//mask portPattern_R0 to contain only bits 12-15 | need W since port is short int (16 bits) (extended with 0s) masked to contain bits 0-11
	//AND R0 and R1
	//AND P0 and R2
	//OR results together
