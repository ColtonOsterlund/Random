...
void Control_Both_Temp_Water(int MAXWATER, int MAXHEAT, COFFEEPOT_DEVICE *pCoffeepot){
	...
	if(currentWaterLevel < (MAXWATER/2)){ //UNDER HALF FULL
		pCoffeepot -> waterInFlowRegister = 0xff; //WATER FULLY ON - 255
	}
	else if(currentWaterLevel >= (MAXWATER/2) && currentWaterLevel < (MAXWATER * 0.8)){ //BETWEEN HALF AND 80% FULL
		pCoffeepot -> waterInFlowRegister = 0xbf; // WATER 3/4 ON - 191
	}
	else if(currentWaterLevel >= (MAXWATER * 0.8) && currentWaterLevel < (MAXWATER * 0.9)){ //BETWEEN 80% AND 90% FULL
	...
	if(currentWaterTemperature < (MAXHEAT/2)){
		pCoffeepot -> heaterRegister = 0xff; //HEAT FULLY ON - 255
		pCoffeepot -> heaterBoostRegister = 0x0f; //HEATER BOOST FULLY ON - 15
	}
	else if(currentWaterTemperature >= (MAXHEAT/2) && currentWaterTemperature < (MAXHEAT * 0.75)){
		pCoffeepot -> heaterRegister = 0xbf; //HEAT 3/4 ON - 191
		pCoffeepot -> heaterBoostRegister = 0x0f; //HEATER BOOST FULLY ON - 15
	}
	else if(currentWaterTemperature >= (MAXHEAT * 0.75) && currentWaterTemperature < (MAXHEAT * 0.9)){
		pCoffeepot -> heaterRegister = 0x80; //HEAT 1/2 ON - 128
		pCoffeepot -> heaterBoostRegister = 0x0f; //HEATER BOOST FULLY ON - 15
	}
	else if(currentWaterTemperature >= (MAXHEAT * 0.9)){
