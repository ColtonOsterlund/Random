...
class BinSearchTree {
	 ...
	 */
	public void insert(int id, String faculty, String major, String year) {
		...
		if(root == null)
			...
			if(cur.data.id >node.data.id)
				cur.left=node;
			else if (cur.data.id <node.data.id)
				cur.right = node;
...
//		splice();
	 ...
	 */	
	public Node find( Node start, int target_id) {
		if(start == null) return null;
		if(start.data.id == target_id) return start;
		if(target_id > start.data.id) 
			return find(start.right, target_id); 
		else if(target_id < start.data.id)
	...
	private Node search(Node cur, Node node) {
		if((cur.left ==null && cur.right ==null)||
			 ((node.data.id < cur.data.id) && (cur.left==null))||
			 ((node.data.id > cur.data.id) && (cur.right==null)))
			return cur;
		if((node.data.id < cur.data.id) && (cur.left != null))
			cur =search(cur.left, node);
		else if((node.data.id > cur.data.id) && (cur.right != null))
			cur =search(cur.right, node);
		else if((node.data.id > cur.data.id) && cur.right!= null)
			cur=search(cur.right, node);
		else if((node.data.id < cur.data.id) && cur.left!= null)
			cur=search(cur.left, node);
		return cur;
	}
